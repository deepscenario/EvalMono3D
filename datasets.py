    ...
    def __init__(self, annotation_files, filter_settings=None):
        ...
        if filter_settings is None:
            ...
            for anno_idx, anno in enumerate(self.dataset['annotations']):
                ...
                if filter_settings['trunc_2D_boxes'] and 'bbox2D_trunc' in anno and not np.all(
                    [val == -1 for val in anno['bbox2D_trunc']]):
                    bbox2D = BoxMode.convert(anno['bbox2D_trunc'], BoxMode.XYXY_ABS, BoxMode.XYWH_ABS)
                ...
                if filter_settings['modal_2D_boxes'] and anno['bbox2D_tight'][0] != -1:
                    self.dataset['annotations'][anno_idx]['bbox'] = BoxMode.convert(anno['bbox2D_tight'],
...
def _load_omni3d_json(json_file, image_root, dataset_name, filter_settings, filter_empty=False):
    # read in the dataset
